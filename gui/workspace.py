...
from db.database import add_bot, clear_logs, delete_bot, get_all_bots, get_bot, get_logs, get_stats, get_total_stats, init_db, update_bot
from gui.components.agent_form import AgentFormPanel
from gui.components.cards import MetricCard, SectionCard, short_time
from gui.theme import APP_BG, BORDER, CARD_ALT, CARD_BG, CARD_HOVER, CLAUDE_MODELS, CONSOLE_BG, CURRENT_VERSION, DANGER, GLOW, PANEL_BG, PRIMARY, PRIMARY_HOVER, RELEASE_NOTES, SECONDARY, SUCCESS, SURFACE_BG, TEXT, TEXT_MUTED, TEXT_SOFT
...
class SeleneApp(ctk.CTk):
    ...
    def _build_shell(self):
        ...
        self.sidebar = ctk.CTkFrame(self, fg_color=SURFACE_BG, corner_radius=0, border_width=0, width=260)
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(brand, text="Control Center for Telegram Userbots", font=("Segoe UI", 11), text_color=TEXT_SOFT, justify="left").pack(anchor="w", pady=(8, 0))

        ctk.CTkFrame(self.sidebar, fg_color=BORDER, height=1).pack(fill="x", padx=20, pady=(0, 16))

        for key, label in [("agents", "Agents"), ("analytics", "Analytics"), ("activity", "Activity"), ("system", "System")]:
            button = ctk.CTkButton(self.sidebar, text=label, anchor="w", height=44, corner_radius=14, fg_color=SURFACE_BG, hover_color=CARD_HOVER, text_color=TEXT, font=("Segoe UI Semibold", 13), command=lambda k=key: self.show_page(k))
        ...
        release_btn = ctk.CTkButton(self.sidebar, text="Release Notes", anchor="w", height=40, corner_radius=14, fg_color=SURFACE_BG, hover_color=CARD_HOVER, text_color=TEXT_MUTED, font=("Segoe UI", 12), command=lambda: self.show_page("release"))
    ...
    def _build_agents_page(self):
        ...
        ctk.CTkButton(hero, text="Create Agent", width=136, height=42, corner_radius=14, fg_color=PRIMARY, hover_color=PRIMARY_HOVER, text_color=APP_BG, font=("Segoe UI Semibold", 12), command=self.start_create_agent).grid(row=0, column=1, rowspan=2, padx=20, pady=18)
        ...
        self.agent_form_view = AgentFormPanel(self.agent_workspace, on_save=self.save_agent, on_cancel=self.exit_agent_form)
    ...
    def _build_analytics_page(self):
        ...
        for idx, (key, title, accent) in enumerate([("agents", "Configured Agents", PRIMARY), ("active", "Active Now", SUCCESS), ("messages", "Messages (7d)", SECONDARY), ("tokens", "Tokens (7d)", SECONDARY)]):
    ...
    def _build_system_page(self):
        ...
        for label, value in [("Interface", "CustomTkinter desktop workspace"), ("Messaging", "Telethon Telegram client API"), ("Language Model", "Anthropic Claude"), ("Storage", "SQLite local database")]:
        ...
        ctk.CTkLabel(notes.body, text="Selene presents Telegram account automation as a neutral operations tool: credentials, prompts, sessions, logs, and analytics are managed in one focused workspace.", text_color=TEXT_MUTED, font=("Segoe UI", 12), justify="left", wraplength=920).pack(anchor="w")
        return page
    ...
    def _build_release_page(self):
        ...
        color_map = {"major": PRIMARY, "patch": SECONDARY, "hotfix": DANGER}
        kind_map = {"new": PRIMARY, "fix": SUCCESS, "change": SECONDARY}
    ...
    def show_page(self, key: str):
        ...
        for name, button in self.nav_buttons.items():
            active = name == key
            button.configure(fg_color=CARD_ALT if active else SURFACE_BG, text_color=TEXT if active else (TEXT_MUTED if name == "release" else TEXT), border_width=1 if active else 0, border_color=BORDER)
    ...
    def _render_agent_list(self, bots):
        ...
        for cfg in bots:
            ...
            state_label = ctk.CTkLabel(row, text="Online" if running else "Standby", text_color=SUCCESS if running else TEXT_SOFT, font=("Segoe UI Semibold", 11))
            state_label.pack(side="right")
            self._bind_card_click([card, row, emblem, text_box, name_label, meta_label, state_label], lambda bot_id=cfg["id"]: self.select_bot(bot_id))
    ...
    def _render_activity_agent_list(self, bots):
        ...
        for cfg in bots:
            running = self.manager.is_running(cfg["id"])
            button = ctk.CTkButton(self.activity_agent_list, text=f"{cfg.get('emoji') or '☾'}  {cfg['name']}", anchor="w", height=42, corner_radius=14, fg_color=CARD_ALT if cfg["id"] == self.selected_bot_id else CARD_BG, hover_color=CARD_HOVER, text_color=TEXT if running else TEXT_MUTED, font=("Segoe UI Semibold", 12), command=lambda bot_id=cfg["id"]: self.select_bot(bot_id))
            button.pack(fill="x", pady=4)

    def _update_sidebar_metrics(self, bots):
        active_count = sum(1 for cfg in bots if self.manager.is_running(cfg["id"]))
        self.sidebar_status_value.configure(text=f"{active_count} active agents")
        self.header_chip_label.configure(text="System Awake" if active_count else "System Calm", text_color=SUCCESS if active_count else PRIMARY)
    ...
    def _highlight_selected_agent(self, bot_id: int):
        for current_id, card in self.agent_cards.items():
            ...
            card.configure(fg_color=CARD_ALT if selected else CARD_BG, border_color=PRIMARY if selected else BORDER, border_width=1 if not selected else 2)
    ...
    def _render_selected_agent(self, cfg):
        ...
        self.selected_meta.configure(text=f"{('Online' if running else 'Standby')}  ·  Uptime {uptime}  ·  Phone {cfg.get('phone', '--')}")
        ...
        self.launch_button.configure(text="Stop" if running else "Launch", fg_color=DANGER if running else SUCCESS, hover_color="#F7ACB4" if running else "#91E7CB")
    ...
    def create_session_for_selected(self):
        ...
        try:
            if sys.platform == "win32":
                subprocess.Popen(["cmd", "/c", "start", "cmd", "/k", sys.executable, script, "--bot-id", bot_id], creationflags=subprocess.CREATE_NEW_CONSOLE)
            else:
    ...
    def refresh_analytics_page(self, bots=None):
        ...
        self._update_metric_card(self.analytics_metric_cards["messages"], str(totals["messages"]), "Messages counted over the last 7 days", SECONDARY)
    ...
    def refresh_system_page(self, bots=None):
        ...
        lines = [f"version: {CURRENT_VERSION}", f"configured_agents: {len(bots)}", f"active_agents: {active}", f"selected_agent: {self.selected_bot_id if self.selected_bot_id is not None else 'none'}", "workspace_mode: single-window", "product_focus: telegram-userbot-management"]
